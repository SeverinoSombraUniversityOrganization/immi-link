...
const PostList = ({ posts, isLoggedUser, setPosts }) => {
  ...
  const handleUpdateLikes = async (id, data) => {
    try {
      ...
      .then((response) => {
        const filteredPosts = response.filter(post => post.userId === props.isLoggedUser.user.id);
  ...
  const renderItem = ({ item }) => (
    <View>
      ...
      <View style={styles.inputView}>
        <TouchableOpacity style={styles.row} onPress={() => handleProfile(item._id, item.username)}>
          <Image
...
export default function ProfileScreen(props) {
    ...
    const { userId, username, userProfilePhoto } = route.params;
